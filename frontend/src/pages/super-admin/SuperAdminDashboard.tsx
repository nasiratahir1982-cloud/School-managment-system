...
import type { SupportedCountry, TenantInfo } from '../../store/tenantStore';
...
export const SuperAdminDashboard: React.FC = () => {
  ...
  const addTenantToStore = useTenantStore((state) => state.addTenant);
  ...
  const [activeDetailModal, setActiveDetailModal] = useState<'tenants' | 'database' | 'queues' | 'cpu' | null>(null);
  ...
  const handleLaunchSchoolPortal = async (tenant: TenantInfo) => {
    try {
      await resolveTenant(tenant.domain);
      loginUser({
        userId: `usr-${Date.now()}`,
        name: `Principal (${tenant.schoolName})`,
        email: `admin@${tenant.domain}.academichub.com`,
        role: 'admin',
  ...
  const handleLaunch = () => {
    if (!schoolName || !domain || !city || !adminEmail) return;
      ...
      setActivities(prev => [
        {
          ...
          time: 'Just now',
          type: 'success',
          message: `🎉 School Portal for ${schoolName} (${city}) registered and live successfully!`
        },
        {
          id: `act-${Date.now()}-2`,
          time: 'Just now',
          type: 'setup',
          message: `👤 Admin account configured for ${adminName || 'Principal'} (${adminEmail}).`
        },
        {
          id: `act-${Date.now()}-3`,
          time: 'Just now',
          type: 'info',
          message: `🎨 Applied brand color scheme to ${cleanDomain}.academichub.com.`
        },
        ...prev
      ]);
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col font-sans transition-colors duration-300">
      ...
      <main className="flex-1 max-w-full w-full px-4 sm:px-8 lg:px-12 py-8 space-y-8">
        ...
        <section className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div 
            onClick={() => setActiveDetailModal('tenants')}
          ...
          >
            ...
            <div>
              <h3 className="text-2xl font-bold text-white">{tenantsList.length}</h3>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-800/60 text-sm text-slate-300">
                  {tenantsList.map((tenant) => (
                    <tr 
                      key={tenant.tenantId} 
                      onClick={() => handleLaunchSchoolPortal(tenant)}
                      className="hover:bg-slate-900/35 transition-colors cursor-pointer group"
                      title={`Launch & Administer ${tenant.schoolName} Portal`}
                    >
                      <td className="px-6 py-4 font-mono text-xs text-slate-400 group-hover:text-purple-400 transition-colors">{tenant.domain}</td>
                      <td className="px-6 py-4 font-bold text-white group-hover:text-purple-300 transition-colors">{tenant.schoolName}</td>
                      <td className="px-6 py-4 flex items-center gap-2">
                        <span>{COUNTRY_CONFIGS[tenant.country]?.flag}</span>
                        <span className="font-semibold text-slate-200">{COUNTRY_CONFIGS[tenant.country]?.countryName}</span>
                      </td>
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <span className="flex items-center gap-1.5">
                            <span 
                              ...
                              style={{ backgroundColor: `hsl(${tenant.themeSettings.primaryHsl.split(' ')[0]} 70% 50%)` }}
                            ></span>
                            <span className="text-xs text-slate-400 font-mono">HSL: {tenant.themeSettings.primaryHsl}</span>
                          </span>
              ...
              <div className="space-y-4 animate-fadeIn">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="346.8 77.2% 49.8%">Crimson Academic</option>
                  </select>
                </div>
              </div>
            )}
                ...
                <div className="space-y-3">
                  <div className="p-3 bg-slate-950 rounded-lg border border-slate-850 space-y-2 text-xs">
                    <div className="flex justify-between text-slate-400">
                      ...
                      <span className="text-white font-bold">{tenantsList.length}</span>
                    </div>
                    <div className="flex justify-between text-slate-400">
                  ...
                  <div className="max-h-60 overflow-y-auto space-y-2 pr-1">
                    {tenantsList.map((t) => {
                      const isExpanded = selectedSchoolDetail === t.tenantId;
                      return (
                        <div 
                          key={t.tenantId} 
                        ...
                        >
                          <div 
                            onClick={() => setSelectedSchoolDetail(isExpanded ? null : t.tenantId)}
                            className="flex items-center justify-between cursor-pointer"
                          >
                            <div className="flex items-center gap-2.5">
                              {t.logoUrl ? (
                                <img src={t.logoUrl} alt="Logo" className="w-8 h-8 rounded-lg object-cover" />
                              ) : (
