...
export const AcademicCalendar: React.FC<AcademicCalendarProps> = ({
  ...
  const renderMonth = (year: number, month: number) => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (isToday) {
        cellClasses += 'ring-2 ring-primary ring-offset-1 ring-offset-background font-black text-primary bg-primary/10 ';
      } else if (blocked) {
        cellClasses += 'opacity-25 text-muted-foreground cursor-default ';
      } else if (inset) {
        cellClasses += 'bg-amber-500/10 text-amber-400 font-bold border border-amber-500/20 ';
      ...
      cells.push(
        <div
          ...
          onClick={() => !blocked && setSelectedDate(dateStr)}
          title={
            blocked
